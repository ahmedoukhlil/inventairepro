<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Demandeurs</h1>
            <p class="text-gray-500 mt-1">Personnes pouvant demander des produits</p>
        </div>
        <a href="<?php echo e(route('stock.demandeurs.create')); ?>"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100">
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Nom</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide hidden sm:table-cell">Poste / Service</th>
                        <th class="px-4 py-3 text-center text-xs font-semibold text-gray-400 uppercase tracking-wide">Sorties</th>
                        <th class="px-6 py-3 text-right text-xs font-semibold text-gray-400 uppercase tracking-wide">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $demandeurs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $demandeur): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="flex-shrink-0 w-9 h-9 rounded-full bg-violet-50 flex items-center justify-center">
                                        <svg class="w-4 h-4 text-violet-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                        </svg>
                                    </div>
                                    <span class="text-sm font-semibold text-gray-800"><?php echo e($demandeur->nom); ?></span>
                                </div>
                            </td>
                            <td class="px-4 py-3 hidden sm:table-cell">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">
                                    <?php echo e($demandeur->poste_service ?: '—'); ?>

                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold
                                    <?php echo e($demandeur->sorties_count > 0 ? 'bg-violet-100 text-violet-700' : 'bg-gray-100 text-gray-500'); ?>">
                                    <?php echo e($demandeur->sorties_count); ?>

                                </span>
                            </td>
                            <td class="px-6 py-3 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="<?php echo e(route('stock.demandeurs.edit', $demandeur->id)); ?>"
                                    ...
                                    <button wire:click="confirmDelete(<?php echo e($demandeur->id); ?>)"
                                            wire:loading.attr="disabled"
                                            ...
                                            class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium transition-colors disabled:opacity-40
                                                   <?php echo e($demandeur->sorties_count > 0
                                                        ? 'bg-gray-100 text-gray-400 cursor-not-allowed pointer-events-none'
                                                        : 'bg-red-50 text-red-600 hover:bg-red-100'); ?>"
                                            <?php if($demandeur->sorties_count > 0): ?> title="Impossible : <?php echo e($demandeur->sorties_count); ?> sortie(s) liée(s)" <?php endif; ?>>
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm font-medium text-gray-500">Aucun demandeur trouvé</p>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($demandeurs->hasPages()): ?>
            <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-4">
                <p class="text-xs text-gray-500">
                    <?php echo e($demandeurs->firstItem()); ?>–<?php echo e($demandeurs->lastItem()); ?> sur <?php echo e($demandeurs->total()); ?> demandeur<?php echo e($demandeurs->total() > 1 ? 's' : ''); ?>

                </p>
                <?php echo e($demandeurs->links()); ?>

            </div>
        <?php else: ?>
            <div class="px-6 py-3 border-t border-gray-50">
                <p class="text-xs text-gray-400"><?php echo e($demandeurs->total()); ?> demandeur<?php echo e($demandeurs->total() > 1 ? 's' : ''); ?></p>
            </div>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    </div>
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 z-10">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Êtes-vous sûr de vouloir supprimer ce demandeur ? Cette action est irréversible.</p>
                    </div>
                </div>
...
<?php /**PATH C:\xampp\htdocs\gesimmos\resources\views/livewire/stock/demandeurs/liste-demandeurs.blade.php ENDPATH**/ ?>