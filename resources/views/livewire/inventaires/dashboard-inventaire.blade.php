<div>
    ...
    <div class="mb-8">
        <nav class="flex mb-4" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3 text-sm">
                <li><a href="{{ route('dashboard') }}" class="text-gray-500 hover:text-indigo-600">Dashboard</a></li>
                <li class="flex items-center"><svg class="w-4 h-4 text-gray-400 mx-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg><a href="{{ route('inventaires.index') }}" class="text-gray-500 hover:text-indigo-600">Inventaires</a></li>
                <li class="flex items-center"><svg class="w-4 h-4 text-gray-400 mx-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg><span class="text-gray-700 font-medium">{{ $inventaire->annee }}</span></li>
            </ol>
        </nav>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p class="text-4xl font-bold text-gray-900 mb-1">{{ round($stats['progression_globale'], 1) }}%</p>
            <div class="w-full bg-gray-100 rounded-full h-2.5 mb-3">
                <div class="bg-indigo-600 h-2.5 rounded-full transition-all duration-700" style="width: {{ min($stats['progression_globale'], 100) }}%"></div>
            </div>
            <div class="flex items-center justify-between text-sm">
                <span class="text-gray-500">{{ $stats['total_biens_scannes'] }}/{{ $stats['total_biens_attendus'] }} scannés</span>
                <span class="text-gray-400">{{ $stats['localisations_terminees'] }}/{{ $stats['total_localisations'] }} localisations</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-1.5 text-sm">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span class="font-medium text-gray-700">{{ $stats['biens_deplaces'] }}</span>
                </div>
                <div class="flex items-center justify-between">
        ...
        <div class="bg-white rounded-xl shadow-sm border {{ $totalAlertes > 0 ? 'border-red-200' : 'border-gray-200' }} p-6">
            ...
            <p class="text-4xl font-bold {{ $totalAlertes > 0 ? 'text-red-600' : 'text-green-600' }} mb-3">{{ $totalAlertes }}</p>
            @if($totalAlertes > 0)
                <p class="text-sm text-gray-500 mb-2">
                    @if($stats['biens_absents'] > 0) {{ $stats['biens_absents'] }} absent(s) @endif
                    @if($stats['biens_absents'] > 0 && ($stats['biens_defectueux'] ?? 0) > 0) &middot; @endif
                    @if(($stats['biens_defectueux'] ?? 0) > 0) {{ $stats['biens_defectueux'] }} défectueux @endif
                </p>
    ...
    {{-- ========================================== --}}
    {{-- SECTION 3 : GRAPHIQUE TEMPOREL (seul)       --}}
    {{-- ========================================== --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h3 class="text-base font-semibold text-gray-900">Progression temporelle</h3>
                <p class="text-sm text-gray-500 mt-0.5">Cumul des scans vs objectif &middot; {{ $stats['vitesse_moyenne'] }} scans/jour en moyenne</p>
            </div>
            @if($stats['scans_aujourdhui'] > 0)
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700">
                    ...
                    {{ $stats['scans_aujourdhui'] }} scan(s) aujourd'hui
                </span>
            @endif
        </div>
        ...
        <div id="alertes-detail" x-data="{ expanded: true }" class="bg-white rounded-xl shadow-sm border border-red-200 mb-8 overflow-hidden">
            <button @click="expanded = !expanded" class="w-full flex items-center justify-between px-6 py-4 hover:bg-red-50/30 transition-colors">
                <div class="flex items-center gap-3">
                    ...
                    <div class="text-left">
                        <p class="text-sm font-semibold text-red-800">{{ $totalAlertes }} alerte(s) requièrent votre attention</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <div class="hidden sm:flex items-center gap-2">
                        @if(count($alertes['localisations_bloquees']) > 0)
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">{{ count($alertes['localisations_bloquees']) }} bloquée(s)</span>
                        @endif
                        @if(count($alertes['biens_absents_valeur_haute']) > 0)
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">{{ count($alertes['biens_absents_valeur_haute']) }} absent(s)</span>
                        @endif
                        @if(count($alertes['localisations_non_assignees']) > 0)
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">{{ count($alertes['localisations_non_assignees']) }} non assignée(s)</span>
                        @endif
                    </div>
    ...
    {{-- ========================================== --}}
    {{-- SECTION 5 : TABLEAU LOCALISATIONS           --}}
    {{-- ========================================== --}}
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <select wire:model.live="filterAgent" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        @foreach($this->agents as $agent)
                            <option value="{{ $agent->idUser }}">{{ $agent->users ?? $agent->name ?? 'Agent' }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($this->inventaireLocalisations as $invLoc)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-600 max-w-[250px] truncate block" title="{{ $invLoc->localisation->Localisation ?? '' }}">{{ $invLoc->localisation->Localisation ?? 'N/A' }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <div class="w-24 bg-gray-100 rounded-full h-2">
                                        <div class="{{ $progressionColor }} h-2 rounded-full transition-all duration-500" style="width: {{ min($progression, 100) }}%"></div>
                                    </div>
                                    <span class="text-sm tabular-nums {{ $progression >= 100 ? 'text-green-600 font-semibold' : 'text-gray-600' }}">
                                    ...
                                    @if($invLoc->agent)
                                        <div class="flex items-center gap-2">
                                            <div class="h-7 w-7 rounded-full bg-indigo-100 flex items-center justify-center flex-shrink-0">
                                                <span class="text-xs font-medium text-indigo-600">{{ mb_substr($invLoc->agent->users ?? $invLoc->agent->name ?? '?', 0, 1) }}</span>
                                            </div>
                                            <span class="text-sm text-gray-700">{{ $invLoc->agent->users ?? $invLoc->agent->name ?? 'Agent' }}</span>
                                        </div>
                                    @else
                                    ...
                                    @if($isAdmin)
                                        <div x-data="{ open: false }" class="relative">
                                            ...
                                            <div x-show="open" @click.away="open = false" x-transition x-cloak class="absolute right-0 mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-20">
                                                ...
                                                @foreach($this->agents as $agent)
                                                    <button wire:click="reassignerLocalisation({{ $invLoc->id }}, {{ $agent->idUser }})" @click="open = false" class="w-full text-left px-3 py-2 text-sm hover:bg-indigo-50 {{ $invLoc->user_id == $agent->idUser ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-gray-700' }}">
                                                        {{ $agent->users ?? $agent->name ?? 'Agent' }}
                                                        @if($invLoc->user_id == $agent->idUser) <svg class="w-3.5 h-3.5 inline ml-1 text-indigo-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg> @endif
                                                    </button>
                                                @endforeach
                                                @if($invLoc->agent)
                                                    <div class="border-t border-gray-100 mt-1"></div>
                                                    <button wire:click="reassignerLocalisation({{ $invLoc->id }}, '')" @click="open = false" class="w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50">Retirer</button>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <a href="{{ route('localisations.show', $invLoc->localisation) }}" class="text-sm text-indigo-600 hover:text-indigo-800 font-medium">Détails</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
