<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        ...
        <a href="{{ route('stock.categories.create') }}"
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-sm font-semibold text-gray-800">{{ $categorie->libelle }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold
                                    {{ $categorie->produits_count > 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500' }}">
                                    {{ $categorie->produits_count }}
                                </span>
                            </td>
                            <td class="px-4 py-3 hidden md:table-cell">
                                <p class="text-sm text-gray-500 truncate max-w-[240px]">
                                    {{ $categorie->observations ? Str::limit($categorie->observations, 60) : '—' }}
                                </p>
                            </td>
                            <td class="px-6 py-3 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('stock.categories.edit', $categorie->id) }}"
                                    ...
                                    <button wire:click="confirmDelete({{ $categorie->id }})"
                                            ...
                                            class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium bg-red-50 text-red-600 hover:bg-red-100 transition-colors disabled:opacity-40
                                                   {{ $categorie->produits_count > 0 ? 'opacity-40 cursor-not-allowed pointer-events-none' : '' }}"
                                            @if($categorie->produits_count > 0) title="Impossible : contient {{ $categorie->produits_count }} produit(s)" @endif>
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if($categories->hasPages())
            <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-4">
                <p class="text-xs text-gray-500">
                    {{ $categories->firstItem() }}–{{ $categories->lastItem() }} sur {{ $categories->total() }} catégorie{{ $categories->total() > 1 ? 's' : '' }}
                </p>
                {{ $categories->links() }}
            </div>
        @else
            <div class="px-6 py-3 border-t border-gray-50">
                <p class="text-xs text-gray-400">{{ $categories->total() }} catégorie{{ $categories->total() > 1 ? 's' : '' }}</p>
            </div>
        @endif
    </div>
