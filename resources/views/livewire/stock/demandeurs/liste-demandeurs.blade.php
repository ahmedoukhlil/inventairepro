<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        ...
        <a href="{{ route('stock.demandeurs.create') }}"
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-sm font-semibold text-gray-800">{{ $demandeur->nom }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 hidden sm:table-cell">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">
                                    {{ $demandeur->poste_service ?: '—' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold
                                    {{ $demandeur->sorties_count > 0 ? 'bg-violet-100 text-violet-700' : 'bg-gray-100 text-gray-500' }}">
                                    {{ $demandeur->sorties_count }}
                                </span>
                            </td>
                            <td class="px-6 py-3 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('stock.demandeurs.edit', $demandeur->id) }}"
                                    ...
                                    <button wire:click="confirmDelete({{ $demandeur->id }})"
                                            wire:loading.attr="disabled"
                                            ...
                                            class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium transition-colors disabled:opacity-40
                                                   {{ $demandeur->sorties_count > 0
                                                        ? 'bg-gray-100 text-gray-400 cursor-not-allowed pointer-events-none'
                                                        : 'bg-red-50 text-red-600 hover:bg-red-100' }}"
                                            @if($demandeur->sorties_count > 0) title="Impossible : {{ $demandeur->sorties_count }} sortie(s) liée(s)" @endif>
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if($demandeurs->hasPages())
            <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-4">
                <p class="text-xs text-gray-500">
                    {{ $demandeurs->firstItem() }}–{{ $demandeurs->lastItem() }} sur {{ $demandeurs->total() }} demandeur{{ $demandeurs->total() > 1 ? 's' : '' }}
                </p>
                {{ $demandeurs->links() }}
            </div>
        @else
            <div class="px-6 py-3 border-t border-gray-50">
                <p class="text-xs text-gray-400">{{ $demandeurs->total() }} demandeur{{ $demandeurs->total() > 1 ? 's' : '' }}</p>
            </div>
        @endif
    </div>
