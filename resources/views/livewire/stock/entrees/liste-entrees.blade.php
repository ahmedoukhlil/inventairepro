            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100">
                        ...
                        <th class="px-4 py-3 text-center text-xs font-semibold text-gray-400 uppercase tracking-wide"></th>
                        @endif
                    </tr>
                </thead>
                    ...
                    @forelse($entrees as $entree)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-3 whitespace-nowrap">
                                <p class="text-sm font-medium text-gray-800">{{ $entree->date_entree->format('d/m/Y') }}</p>
                            </td>
                            <td class="px-4 py-3">
                                <p class="text-sm font-semibold text-gray-800">{{ $entree->produit->libelle ?? '—' }}</p>
                                @if($entree->produit?->categorie)
                                    <p class="text-xs text-gray-400">{{ $entree->produit->categorie->libelle }}</p>
                                @endif
                            </td>
                            <td class="px-4 py-3 hidden md:table-cell">
                                <p class="text-sm text-gray-700">{{ $entree->fournisseur->libelle ?? '—' }}</p>
                            </td>
                            <td class="px-4 py-3 hidden lg:table-cell">
                                <p class="text-sm text-gray-500 font-mono">{{ $entree->reference_commande ?? '—' }}</p>
                            </td>
                            ...
                            <td class="px-6 py-3 hidden sm:table-cell">
                                <p class="text-sm text-gray-500">{{ $entree->nom_createur }}</p>
                            </td>
                            @if(auth()->user()->canDeleteStockOperations())
                            <td class="px-4 py-3 text-center">
                                <button
                                    wire:click="supprimer({{ $entree->id }})"
                                    wire:confirm="Supprimer cette entrée ? Le stock sera diminué automatiquement."
                                    wire:loading.attr="disabled"
        ...
        @if($entrees->hasPages())
            <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-4">
                <p class="text-xs text-gray-500">
                    {{ $entrees->firstItem() }}–{{ $entrees->lastItem() }} sur {{ $entrees->total() }} entrée{{ $entrees->total() > 1 ? 's' : '' }}
                </p>
                {{ $entrees->links() }}
            </div>
        @else
            <div class="px-6 py-3 border-t border-gray-50">
                <p class="text-xs text-gray-400">{{ $entrees->total() }} entrée{{ $entrees->total() > 1 ? 's' : '' }}</p>
            </div>
        @endif
    </div>
</div>
